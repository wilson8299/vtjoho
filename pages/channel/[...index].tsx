...
import { LiveCard, VideoCard, Pagination, FavoriteButton } from "@/components";
...
const Channel: React.FC<IBaseProps> = ({ userId, member }) => {
  ...
  return (
    <section className="flex h-full flex-col overflow-y-auto bg-light-300 dark:bg-dark-300">
      {pageVideoData && loading && (
      ...
      <div className=" grow bg-light-200 py-6 dark:bg-dark-200">
        <div className="container mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-[repeat(auto-fill,minmax(300px,1fr))]">
            {!favoriteIsLoading &&
              live?.map((live, index) => (
                <LiveCard key={index} {...live} memberInfo={{ ...member }} />
              ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-[repeat(auto-fill,minmax(300px,1fr))]">
            {!favoriteIsLoading &&
              pageVideoData?.data?.map((video, index) => (
                <VideoCard key={index} {...video} memberInfo={{ ...member }} />
              ))}
          </div>
        </div>
        <div>
